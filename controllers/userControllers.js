...
const userControllers = {
  addUser: async (req, res) => {
    let { email, password, userName, lastName, country, imgUrl, googleFlag } =
      req.body;
    ...
    try {
      ...
      if (userExists) {
        res.json({
          success: false,
          error: true,
          answer: [{ message: "This email is already registered" }],
        });
      } else {
    ...
    try {
      ...
      if (!userExists) {
        res.json({
          success: false,
          error: true,
          answer: [{ message: "Email is incorrect" }],
        });
      } else {
      ...
      if (userExists.googleFlag && !googleFlag)
